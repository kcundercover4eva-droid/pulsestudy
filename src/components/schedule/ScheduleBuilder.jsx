...
export default function ScheduleBuilder() {
  ...
  const [currentMobileDay, setCurrentMobileDay] = useState(0); // 0 = Monday
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-6">
      ...
      {/* Scheduler Grid - Mobile: single day view with vertical time layout */}
      <div className="md:hidden flex-1 glass-card rounded-3xl overflow-hidden relative shadow-2xl flex flex-col">
        {/* Day Navigation Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-black/40 backdrop-blur-xl">
          <button 
            onClick={() => setCurrentMobileDay(prev => prev === 0 ? 6 : prev - 1)}
          ...
          <div className="text-center">
            <h3 className="text-xl font-bold text-white">{DAYS[currentMobileDay]}</h3>
            <p className="text-xs text-white/40">Tap arrows to change day</p>
          </div>
          
          <button 
            onClick={() => setCurrentMobileDay(prev => prev === 6 ? 0 : prev + 1)}
            className="p-2 rounded-lg hover:bg-white/10 transition-colors"
          >
            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden bg-slate-900/50">
          <div className="flex relative" style={{ height: HOURS.length * PIXELS_PER_HOUR }}>
            ...
            <div 
              ...
              onPointerDown={(e) => handleGridPointerDown(e, currentMobileDay)}
            >
              {/* Hourly Lines */}
              {HOURS.map(h => (
                <div 
              ...
              {/* Creation Ghost Block */}
              {dragState?.action === 'create' && dragState.day === currentMobileDay && (
                <div
                  className="absolute left-1 right-1 rounded-xl border-2 border-cyan-400 border-dashed z-50 pointer-events-none flex items-center justify-center"
                  style={{
                    top: `${dragState.start * PIXELS_PER_HOUR}px`,
              ...
              {/* Blocks */}
              <AnimatePresence>
                {localBlocks.filter(b => b.day === currentMobileDay).map((block) => (
                  <motion.div
                    layout
                    key={block.id}
                    initial={{ opacity: 0, scale: 0.9 }}
